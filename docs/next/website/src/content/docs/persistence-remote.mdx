...
Use `transport = "ssh"` when the remote host cannot accept UDP. Use `transport = "quic"` with `ssh_fallback = false` when QUIC is required and failure should be explicit instead of falling back.
