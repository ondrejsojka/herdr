...
[remote]
# Live transport preference: auto (QUIC raced against the SSH bridge), quic
# (QUIC only, no SSH stream fallback), or ssh.
# transport = "auto"
...
# Transparently use the SSH stdio bridge when UDP is unavailable (auto only).
# ssh_fallback = true
