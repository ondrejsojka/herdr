...
use crate::server::client_transport::{
    ...
    MAX_INPUT_EVENT_BATCH, MAX_INPUT_PAYLOAD, MAX_PIXEL_MOUSE_PAYLOAD,
};
            ...
            ClientMessage::InputPixels {
                ...
                let geometry =
                    crate::input::mouse::HostGeometry::new(cols, rows, width_px, height_px)
                        .ok_or_else(|| "remote pixel mouse geometry is invalid".to_owned())?;
                if data.len() > MAX_PIXEL_MOUSE_PAYLOAD
                    || crate::input::mouse::parse_report(&data).is_none()
                {
                    return Err("remote pixel mouse report is invalid".to_owned());
                }
...
fn control_policy(data: &[u8]) -> ControlPolicy {
    ...
    match message {
        ...
        Ok(ServerMessage::Notify { .. } | ServerMessage::Clipboard { .. }) => ControlPolicy {
            key: None,
