...
pub(crate) struct HandoffManifest {
    ...
    /// Live QUIC transport state. Carrying it means remote clients reconnect to
    /// the successor with the capability they already hold instead of running a
    /// fresh SSH bootstrap. Absent from manifests written before this field
    /// existed and from servers that never bound a QUIC endpoint.
    #[serde(default)]
